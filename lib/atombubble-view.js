  ...
  constructor(serializedState) {
    ...
    if (fs.existsSync(currentDirectory+"/bubblesave.json")) { //if json file exist
      ...
      loadedbubblelinks.forEach((q,i)=>{
        q.forEach((p,j)=>{
          loadedworkingset.forEach((k)=>{
            k.find((obj)=>{
              if (loadedbubblelinks[i][j].start === obj.id) {
                loadedbubblelinks[i][j].start = obj;
              }
              if (loadedbubblelinks[i][j].end === obj.id) {
                loadedbubblelinks[i][j].end = obj;
              }
            });
          });
        });
