    ...
    if (fs.existsSync(currentDirectory+"/bubblesave.json")) { //if json file exist
      ...
      loadedworkingset.forEach((n,i)=>{
        n.forEach((m,j)=>{
          ...
          let openPromise = Promise.resolve(atom.workspace.open(fileToOpen,{activatePane:false,activateItem:false}));
          let filemodel = atom.workspace.getTextEditors().filter(x=>x.getTitle() === m.filename)[0];

          loadedworkingset[i][j].model = filemodel; //load model back to workingset
        });
      });
      //load model back to bubblelinks
      loadedbubblelinks.forEach((q,i)=>{
        q.forEach((p,j)=>{
          let filemodelstart = atom.workspace.getTextEditors().filter(x=>x.getTitle() === p.start.filename)[0];
          let filemodelend = atom.workspace.getTextEditors().filter(x=>x.getTitle() === p.end.filename)[0];

          loadedbubblelinks[i][j].start.model = filemodelstart;
          loadedbubblelinks[i][j].end.model = filemodelend;
        });
      });
    }

    render(<Main workingset={loadedworkingset} bubblelinks={loadedbubblelinks}/>,this.element);
  }
