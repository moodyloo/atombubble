...
export default class Bubble extends React.Component {
  ...
  changeDetect(originalmarker,bubblemodel) {
    var clone,original;
    var marker = originalmarker;

    if (Object.keys(marker).length !== 0) { //if marker exist
      //assigning rows and columns of start and end point in buffer
      let startRow = marker.getBufferRange().start.row;
      let endRow = marker.getBufferRange().end.row;
      let newRange = [[startRow,0],[endRow+1,0]];

      let cloneStartRow = bubblemodel.getBuffer().getRange().start.row;
      let cloneEndRow = bubblemodel.getBuffer().getRange().end.row;
      let cloneRange = [[cloneStartRow,0],[cloneEndRow+1,0]];

      console.log(this.props.originalmodel.getTextInBufferRange(newRange));
      console.log(bubblemodel.getTextInBufferRange(cloneRange));
      //detect change in bubble, update original text editor if change
      clone = bubblemodel.onDidStopChanging(()=>{
        if (this.props.originalmodel.getTextInBufferRange(newRange) !== bubblemodel.getTextInBufferRange(cloneRange)) {
          this.props.originalmodel.setTextInBufferRange(newRange,bubblemodel.getTextInBufferRange(cloneRange));
        }
      });
      //detect change in original text editor, if change edit bubble
      original = this.props.originalmodel.onDidSave(()=>{
        bubblemodel.setTextInBufferRange(cloneRange,this.props.originalmodel.getTextInBufferRange(newRange));
      });
    ...
    this.setState({clone,original,marker});
  }
  ...
  componentDidMount() {
      ...
      if (Object.keys(marker).length !== 0) {
        bubblemodel.insertText(this.props.originalmodel.getTextInBufferRange(marker.getBufferRange()));
      } else {
      ...
      this.changeDetect(marker,bubblemodel);
  }
  ...
  componentWillUnmount() {
    ...
    if (Object.keys(this.props.marker).length !== 0) {this.props.marker.destroy();}
    this.interact.unset();
