...
export default class Bubble extends React.Component {
  ...
  findFunctionRange(text) {
    var parsedtext = acorn.parse(text);
    var selectedfunction = parsedtext.body.filter(x => x.id.name === this.props.select);
    var startrow = selectedfunction[0].start;
    var endrow = selectedfunction[0].end;
  ...
  componentDidMount() {
      ...
      bubblemodel.addGutter({name: "debugger-breakpoint"});
      bubblemodel.addGutter({name: "diagnostics-gutter"});
      console.log(bubblemodel.getGutters());
