...
export default class Bubble extends React.Component {
  ...
  findFunctionRange(text) {
    var error = false;
    var parsedtext;
    try {parsedtext = acorn.parse(text,{locations:true});} catch (e) {error = !error;console.log(e);}
    if (error) {
  ...
  componentDidMount() {
      ...
      //add gutters
      bubblemodel.addGutter({name:"debugger-breakpoint",priority:-1100});
      bubblemodel.addGutter({name:"diagnostics-gutter",priority:-1000});
      console.log(bubblemodel);
