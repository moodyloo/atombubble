...
export default class Bubble extends React.Component {
  ...
  componentDidMount() {
      ...
      if (Object.keys(marker).length !== 0) { //if marker exist
        ...
        clone = bubblemodel.onDidStopChanging(()=>{
          if (this.props.originalmodel.getTextInBufferRange(marker.getBufferRange()) !== bubblemodel.getText()) {
            this.props.originalmodel.setTextInBufferRange(marker.getBufferRange(),bubblemodel.getText());
          }
        });
        //detect change in original text editor, if change edit bubble
        original = this.props.originalmodel.onDidSave(()=>{
          console.log(this.props.originalmodel.getTextInBufferRange(marker.getBufferRange()));
          bubblemodel.setText(this.props.originalmodel.getTextInBufferRange(marker.getBufferRange()));
        });
  ...
  getMenu() {
    return (
      <StyledHandle ref={this.ref}>
        <StyledRemoveIcon id={"delete"+this.props.unique} className="icon icon-remove-close" onClick={(e)=>{this.props.removeBubble(e,this.props.unique)}}/>
