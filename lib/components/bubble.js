...
export default class Bubble extends React.Component {
  ...
  findFunctionRange(text, bmodel) {
    var error = false;
    var parsedtext;
    try { parsedtext = acorn.parse(text, { locations: true }); }
    catch (e) {
      error = !error;
      var row = e.loc.line - 1;
      var col = e.loc.column - 1;
    ...
    if (error) {
      parsedtext = acorn_loose.parse(text, { locations: true });
      error = !error;
    }
    var selectedfunction, startrow, startcol, endrow, endcol;
    try { selectedfunction = parsedtext.body.filter(x => x.id.name === this.props.select) }
    catch (e) {
      error = !error;
    }
    if (error) {
  ...
  render() {
    ...
    return (
      <StyledBubble ref={this.ref} colour={this.props.colour} border={this.state.border} id={"border" + this.props.unique}>
