...
export default class Main extends React.Component {
  ...
  linkBubbles(index) {
    ...
    if (this.state.currentlink === "") { //selecting first bubble for linking
      ...
      if (this.state.currentlink.id === selectedbubble.id) { //if same bubble selected
        ...
        this.setState(prev=>({
          currentlink: "",
          bubblelinks: [...prev.bubblelinks,bubblepair]
        }));
      }
    }
  }
  ...
  removeBubble(e,i) {
    if (e.button == 0) { //if left mouse click
      ...
      bubblelinkscopy = bubblelinkscopy.filter(x=>{ //delete any links between this bubble and others
  ...
  render() {
    {console.log(this.state.bubblelinks)}
    return (
