...
export default class Main extends React.Component {
  ...
  removeBubble(e,bubbleIndex) {
    ...
    if (e.button == 0) { //if left mouse click
      ...
      var newcolour = this.state.colour.slice();
      var newmarker = this.state.markerarray.slice();
      newmarker.splice(bubbleIndex,1);
      newcolour.splice(bubbleIndex,1);
      neworiginal.splice(bubbleIndex,1);
      console.log(neworiginal)
      this.setState({originalarray: neworiginal,colour: newcolour,markerarray:newmarker});
    }
  }
