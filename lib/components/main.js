...
export default class Main extends React.Component {
  ...
  addBubble(originalmod,selected) {
    ...
    this.setState((prev)=>({
      bubblearray: [...prev.bubblearray,
        {id:uuidv4(),model:originalmod,colour:bubbleColour,func:selected,filename:originalmod.getTitle()}]
    }));
  }
  ...
  removeBubble(e,i) {
    if (e.button == 0) { //if left mouse click
      this.setState((prev)=>({
        bubblearray: prev.bubblearray.filter(x => x.id !== i)
      }));
    }
  }
  ...
  render() {
    return (
      <StyledMain>
        <MainSection removeBubble={this.removeBubble} bubblearray={this.state.bubblearray} width={this.state.mainSectionWidth}/>
