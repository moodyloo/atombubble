...
export default class Main extends React.Component {
  ...
  saveFile(e) {
    if (e.button === 0) {
      ...
      var currentDirectory = glob.sync(atom.project.getPaths()[0])[0];
      var setcopy = this.state.workingset.map((x)=>{
        return(x.map((y)=>{
      ...
      //set model property in bubblelinks to empty string
      var linkbubblescopy = this.state.bubblelinks.map((x)=>{
        return(x.map((y)=>{
            var copy = Object.assign({},y);
            return copy;
        }));
      });
      linkbubblescopy.forEach((x,i)=>{
        x.forEach((y,j)=>{
          linkbubblescopy[i][j].start = linkbubblescopy[i][j].start.id;
          linkbubblescopy[i][j].end = linkbubblescopy[i][j].end.id;
        });
      });
      writeJsonFile.sync(currentDirectory+"/bubblesave.json",{workingset: setcopy,bubblelinks: linkbubblescopy});
      console.log("saved!");
    }
  }
